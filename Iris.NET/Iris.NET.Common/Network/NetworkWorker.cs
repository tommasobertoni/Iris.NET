...
namespace Iris.NET
{
    ...
    public class NetworkWorker
    {
        ...
        /// </summary>
        protected static readonly double _MinDelayValueMillis = 1;

        /// <summary>
        /// Factor used to increase the delay for the loop in the writer thread
        /// </summary>
        protected static readonly double _IncrementalDelayFactor = 1.2;
        #endregion
        ...
        private void Listen()
        {
            ...
            while (_isAlive)
            {
                ...
                if (incomingDataMemoryStream.Length >= _SentenceChunksSize) // If there's enough data for a Sentence
                {
                    incomingDataMemoryStream.Seek(0, SeekOrigin.Begin); // Reset position

                    while ((incomingDataMemoryStream.Length - incomingDataMemoryStream.Position) >= _SentenceChunksSize) // While there's enough data for a Sentence
                    {
                        currentChunkMemoryStream.SetLength(0); // Clear the current chunk memory

                        byte[] current = new byte[_SentenceChunksSize];
                        incomingDataMemoryStream.Read(current, 0, _SentenceChunksSize); // Copy into current raw data (of constant size)

                        currentChunkMemoryStream.Write(current, 0, _SentenceChunksSize); // Write raw data to the current chunk memory
