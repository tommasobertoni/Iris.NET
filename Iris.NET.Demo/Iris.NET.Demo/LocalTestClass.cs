...
namespace Iris.NET.Demo
{
    class LocalTestClass
    {
        ...
        private IrisLocalNode localNode1 = new IrisLocalNode();

        private IrisLocalNode localNode2 = new IrisLocalNode();
        private IrisLocalNode localNode3 = new IrisLocalNode();
        ...
        public List<IDisposableSubscription> UseCase_BroadcastCommunication()
        {
            ...
            Task<IDisposableSubscription> subscription1 = localNode1.SubscribeToBroadcast((c, h) => GenericContentHandler(nameof(localNode1), "broadcast", c, h));
            bool result1 = subscription1 != null;
            Task<IDisposableSubscription> subscription2 = localNode2.SubscribeToBroadcast((c, h) => GenericContentHandler(nameof(localNode2), "broadcast", c, h));
            bool result2 = subscription2 != null;

            CheckThatEverythingIsOk(result1, result2);
            disposableSubscriptions.Add(subscription1.Result);
            disposableSubscriptions.Add(subscription2.Result);
            ...
            Task<bool> publishToBroadcastTask = localNode1.PublishToBroadcast(message);
            result1 = publishToBroadcastTask.Result;
        ...
        public List<IDisposableSubscription> UseCase_ChatRoom()
        {
            ...
            Task<IDisposableSubscription> subscription1 = localNode1.Subscribe(_channel, (c, h) => GenericContentHandler(nameof(localNode1), _channel, c, h));
            bool result1 = subscription1 != null;

            Task<IDisposableSubscription> subscription2 = localNode2.Subscribe(_channel, (c, h) => GenericContentHandler(nameof(localNode2), _channel, c, h));
            bool result2 = subscription2 != null;

            Task<IDisposableSubscription> subscription3 = localNode3.Subscribe(_channel, (c, h) => GenericContentHandler(nameof(localNode3), _channel, c, h));
            ...
            disposableSubscriptions.Add(subscription1.Result);
            disposableSubscriptions.Add(subscription2.Result);
            disposableSubscriptions.Add(subscription3.Result);

            string message = $"Hi! my name is {nameof(localNode1)}";
            Console.WriteLine($"{nameof(localNode1)} sends \"{message}\" to the \"{_channel}\" channel.");
            Task<bool> publishTask1 = localNode1.Publish(_channel, message);
            result1 = publishTask1.Result;
            CheckThatEverythingIsOk(result1);
        ...
        public List<IDisposableSubscription> UseCase_NestedChannelsSubscriptions()
        {
            ...
            Console.WriteLine($"{nameof(localNode1)} sends \"{message}\" to the \"{_channel}\" channel.");
            Task<bool> publishTask = localNode1.Publish(_channel, message);
            bool result1 = publishTask.Result;
            CheckThatEverythingIsOk(result1);

            WaitALittle();
            Console.WriteLine();

            Task<IDisposableSubscription> subscription1 = localNode1.Subscribe($"{nestedChannelHierarchy}", (c, h) => GenericContentHandler(nameof(localNode1), nestedChannelHierarchy, c, h));
            result1 = subscription1 != null;

            Task<IDisposableSubscription> subscription3 = localNode3.Subscribe($"{nestedChannelHierarchy}", (c, h) => GenericContentHandler(nameof(localNode3), nestedChannelHierarchy, c, h));
            bool result3 = subscription3 != null;
            ...
            publishTask = localNode1.Publish(nestedChannelHierarchy, message);
            result1 = publishTask.Result;
            CheckThatEverythingIsOk(result1);
        ...
        public void UseCase_FullHierarchyCommunication()
        {
            ...
            Task<bool> publishTask = localNode1.Publish(_channel, message, propagateThroughHierarchy: true);
            bool result1 = publishTask.Result;
            CheckThatEverythingIsOk(result1);
        }
        ...
        public List<IDisposableSubscription> UseCase_ComplexCommunication()
        {
            ...
            Task<IDisposableSubscription> subscription1 = localNode1.Subscribe($"{localNode1.Id.ToString()}", (c, h) => GenericContentHandler(nameof(localNode1), localNode1.Id.ToString(), c, h));
            bool result1 = subscription1 != null;

            Task<IDisposableSubscription> subscription2 = localNode2.Subscribe($"{localNode2.Id.ToString()}", (c, h) => GenericContentHandler(nameof(localNode2), localNode2.Id.ToString(), c, h));
            bool result2 = subscription2 != null;

            Task<IDisposableSubscription> subscription3 = localNode3.Subscribe($"{localNode3.Id.ToString()}", (c, h) => GenericContentHandler(nameof(localNode3), localNode3.Id.ToString(), c, h));
            bool result3 = subscription3 != null;
            ...
            Task<bool> publishTask = localNode1.Publish(localNode2.Id.ToString(), d);
            result1 = publishTask.Result;
            CheckThatEverythingIsOk(result1);
            ...
            publishTask = localNode2.Publish(localNode3.Id.ToString(), obj);
            result2 = publishTask.Result;
        ...
        public void Unsubscribe()
        {
            ...
            if (_IDontBelieveIt)
            {
                ...
                localNode1.PublishToBroadcast(message);
                localNode1.Publish(_channel, message);
                localNode1.Publish($"{_channel}/{_nestedChannelName}", message);
                WaitALittle();
        ...
        private void WaitALittle(int delay = 300)
        {
            Thread.Sleep(delay); // Gives some time to the asyncronous operations to complete
        }
        #endregion
    }
